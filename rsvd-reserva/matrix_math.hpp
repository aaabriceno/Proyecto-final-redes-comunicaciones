...
void generate_omega_mmap(uint64_t seed, int n, int k, const std::string &fileOmega) {
    ...
    std::cout << "[worker] Omega builded: " << fileOmega << "\n";
}

void matmul_A_Omega_mmap(const std::string &fileA,int rows, int cols,
                         const std::string &fileOmega,int k,
    ...
    std::cout << "[worker] Y builded: " << fileY << "\n";
}


void qr_mmap(const std::string &fileY,int rows, int k,
                 const std::string &fileQi,const std::string &fileRi){
...
void compute_Vj_mmap(const std::string &UtilFile,const std::string &SigmaInvFile,
    const std::string &BiFile,const std::string &VjOutFile,int k, int ncols){
    ...
    // Scale rows by SigmaInv: Vj[i,:] *= SigmaInv[i]
    for (int i = 0; i < k; ++i) {
...
void compute_Ui_mmap(const std::string &Qi_file,const std::string &Utilde_file,
    const std::string &Ui_file,int mi, int k){
    ...
    std::cout << "[worker] computed Ui = Q_i * Utilde and stored in "
    << Ui_file << " (" << mi << " x " << k << ")\n";
}
